...
time_window = [-0.4  0.8];
baseline_correction_interval = [-200,-100];
shift = 0; % in seconds
savedir = './preprocessed_data/preprocessed_data_new_200to100';
mkdir(savedir);
...
for session = 3:3
    for subject = 16:29
        ...
        strsub = sprintf('%0*d',(subject<0)+2,subject);
        dir = sprintf(['./raw_data/sub-%s/ses-S%d/eeg/'],strsub, session );
        

        filename = 'Flanker.set';

        save_name = sprintf('%s_sub-%s_ses-S%d', filename, sprintf('%0*d',(subject<0)+2,subject), session);
        filename_before_epoch = char(append(save_name, '_before_epoch.set'));

        check_file_path = fullfile(savedir, filename_before_epoch);

        if isfile(check_file_path)
            fprintf('%s%s already preprocessed!\n', dir, filename);
            %continue;
        end

        % Preprocessing
        EEG.etc.eeglabvers = '2024.2'; % this tracks which version of EEGLAB is being used, you may ignore it
        EEG = pop_loadset(filename, dir);
        EEG = pop_select( EEG, 'rmchannel',{'ECG1'});
        ...
        filename_before_epoch = char(append(filename, '_before_epoch.set'));
        EEG = pop_saveset( EEG, 'filename',sprintf('%s', filename_before_epoch),'filepath',savedir);
